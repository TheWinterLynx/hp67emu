    ...
    pub fn paint(ui: &Ui, host: Rect) {
        ...
        for key in KEYS {
            ...
            let press = ui.ctx().animate_bool_with_time(
                ui.make_persistent_id(("hp67-rigid-key-v3",key.id)),
                target,
                0.050,
            );
...
impl Transform {
    ...
    fn translated_y(self,dy:f32)->Self { Self { origin: Pos2::new(self.origin.x,self.origin.y+dy*self.scale), scale:self.scale } }
}
...
fn draw_key(p:&Painter,t:Transform,key:KeySpec,press:f32) {
    ...
    let travel_px=(2.8*press*t.scale).round();
    let travel=if t.scale>0.0 { travel_px/t.scale } else { 0.0 };
    let kt=t.translated_y(travel);
    ...
    p.rect_stroke(top,kt.s(2.0),Stroke::new(kt.s(0.62),Color32::from_rgba_premultiplied(0,0,0,62)));

    let front_rect=kt.rect(x+0.8,skirt_top,key.w-1.6,skirt_h);
    p.rect_filled(front_rect,kt.s(0.8),front);
    p.rect_filled(kt.rect(x+0.8,skirt_top+0.5,1.15,skirt_h-0.7),0.0,side);
    p.rect_filled(kt.rect(x+key.w-1.95,skirt_top+0.5,1.15,skirt_h-0.7),0.0,darker(side,22));

    p.line_segment(
        [kt.pos(x+2.3,key.y+1.05),kt.pos(x+key.w-2.3,key.y+1.05)],
        Stroke::new(kt.s(0.92),Color32::from_rgba_premultiplied(255,255,244,150)),
    );
    p.line_segment(
        [kt.pos(x+1.8,skirt_top+0.35),kt.pos(x+key.w-1.8,skirt_top+0.35)],
        Stroke::new(kt.s(0.66),Color32::from_rgba_premultiplied(255,255,238,100)),
    );
