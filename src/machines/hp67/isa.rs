...
//! ACT sends the 12-bit ROM address LSB-first during b16..b27; the selected ROM
//! returns a 10-bit word LSB-first during b46..b55.  These helpers encode those
//! electrical/serial facts without yet choosing the exact PHI launch/sample edge.
...
/// role.  Other HP-67 uses of IS, such as ROM0 display-anode data, are separate
/// device behavior and are intentionally not encoded by this helper.
...
mod tests {
    ...
    #[test]
    fn address_and_rom_response_do_not_drive_the_fetch_bus_at_the_same_time() {
        for bit in 0..56 {
            let act = act_address_drive(0x0fff, bit);
            let rom = rom_word_drive(0x03ff, bit);
            assert!(!(act == Drive::High && rom == Drive::High));
        }
    }
}
