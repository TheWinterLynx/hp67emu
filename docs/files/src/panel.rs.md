...
WASM touch ergonomics are presentation-only. `Hp67Panel::show()` accepts a minimum interaction target supplied by the host layout. Compact browser mode requests 44 points; native desktop passes zero and therefore retains the measured photograph hit rectangles exactly. Key, POWER and RUN/PRGM interaction rectangles may grow around their existing visual centres, but the photographed geometry, key identity and physical contact lifetime do not change. When expanded key targets overlap on very narrow screens, the pointer-down capture selects the containing key whose target centre is nearest the touch point, avoiding source-order bias. `height_for_width()` exposes the photograph's fixed aspect ratio so the WASM landscape shell can size a scrollable panel without duplicating panel geometry.
