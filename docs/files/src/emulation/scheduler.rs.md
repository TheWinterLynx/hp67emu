...
Uses `emulation::device` for the `ElectricalDevice`, `NetReader` and `DriveSink` contracts, `emulation::net` for resolved wires and named drivers, and `emulation::clock::Tick` for deterministic time. HP-67 machine code will populate it with ACT, ROM/RAM, display and CRC devices as those implementations are added.
...
`ElectricalScheduler` stores nets in a `BTreeMap` and devices in a vector. `step()` first rejects pre-existing contention, snapshots all resolved levels, evaluates every device against that snapshot into per-device drive buffers, merges those buffers while checking driver identity collisions, commits the complete pending drive set, advances the tick, and verifies the resulting nets are contention-free. Tests prove one-tick propagation, device-order independence, deterministic contention diagnostics and collision rejection before commit.
