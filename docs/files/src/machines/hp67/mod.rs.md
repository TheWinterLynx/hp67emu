...
## Implementation
Declares `act`, `architectural`, `crc`, `display`, `fetch`, `isa`, `machine`, `timing` and `wiring`; re-exports the HP-67 electrical backplane, structural serial fetch runner/endpoints/pipeline latch, complete ACT architectural state/core/RAM scaffold, CRC control decoder/state, ACT+CRC architectural composition, ROM0 display-byte receiver/decoder, explicit `Rom0StrEvent`, downstream `CathodeScanError`/`1820-1749` cathode state, temporary `PowerOnAct*` compatibility aliases, 4/14/56 geometry, evidenced `b0..b7` display window, `b16..b27` address and `b46..b55` ROM-word windows, IS wired-high serializers, chip/net vocabulary and `CHIPSET`.
