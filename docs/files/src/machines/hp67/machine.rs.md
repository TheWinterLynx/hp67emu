...
`commit_staged()` and `advance_clock_edge()`/timing progression are intentionally separate. The backplane no longer equates every electrical commit with one PHI transition.
