...
TeenixHppImport XOR-decodes bytes with 0x55, validates the NeWe envelope and calculator identifier, strips the historical three dummy 28-bit records, validates and removes the duplicate checksum record, and reconstructs the 34 real 28-bit words. The checked-in Teenix card corpus serializes each seven-nibble record least-significant nibble first; after reconstruction, the firmware-visible high nibble of the first 28-bit word is the header ID and is authoritative for Track 1 versus Track 2. Teenix metadata remains host/import metadata and never reaches the transport or CRC.
...
Teenix line endings: the importer accepts LF, CRLF and legacy CR-only separators in the NeWe envelope. The separator parser consumes CRLF as one logical line ending while preserving the raw body byte count used by the declared-length diagnostic. This is required by the checked-in DemoPac1 corpus, whose decoded files use CR-only separators; it does not relax magic, calculator, nibble-count, checksum-mirror or header validation.
