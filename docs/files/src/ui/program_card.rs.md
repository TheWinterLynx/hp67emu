...
PDF-derived face textures: `ProgramCardView::face_texture` optionally carries a raster card face generated from an original pack manual. The renderer maps that texture over the same chamfered physical-card polygon used by the procedural face. End-2 reinsertion flips the texture UVs by 180 degrees, so exact manual artwork follows the same physical orientation rule as labels, marks and the HP logo. When no face texture is present, the existing procedural rendering path remains unchanged.
