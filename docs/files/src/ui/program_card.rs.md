...
Final right-edge registration follows the photographed rail slope instead of a vertical clip. The card is allowed to render to source x=796, then the original `hp67.png` right rail is restored as one texture-mapped trapezoid whose inner edge interpolates from x=787 at the top of the holder to x=792 at the bottom. This replaces the earlier 109 horizontal source bands, which could leave one-pixel sampling seams visible as tiny lines into the casing. The dark slot backing is likewise a trapezoid and stops 0.75 source pixel before the photographed frame boundary, so it cannot cover the thin white rail highlight. The physical card centre remains fixed at source x=465.5 with a -1.5 source-pixel window-centre compensation, so title/logo/A-E registration remains unchanged.
...
Opposite-end presentation on the reader path is physical rather than symbolic. `ProgramCardView::rotated_180` rotates all orientation-bearing artwork by 180 degrees while the card is waiting at the reader, traversing it, or parked at the left exit: top-edge marks move to their mirrored bottom positions, text is emitted through egui 0.27.2 `TextShape::with_angle(PI)`, and the HP logo mesh is rotated about its center. The passive A-E holder intentionally normalizes the face to upright: both the holder-entry animation and `InWindow` rendering pass `rotated_180=false`. This changes presentation only; logical `CardInsertionEnd` remains owned by `app.rs` and continues to choose the physical magnetic track. The card outline itself is invariant under the 180-degree transformation. Tests lock the point/mark transform as an involution.

Reader-entry recovery now has an explicit `WaitingAtReader` presentation state between host insertion and motorized transport. The card is shown protruding at the right reader mouth while `card_present` is asserted but firmware has not yet asserted `motor_on`. That state is not animated. Right-clicking the exposed card requests withdrawal; only a real firmware motor request advances presentation to `ReadingFromRight`. This prevents a card inserted while the calculator is in `Error`, OFF, or another non-reader state from becoming an invisible/stuck zero-progress transport.

Waiting-reader hitbox: the right-click withdrawal target is the bounding union of the photographed reader hotspot and the visible portion of the waiting card, both clipped to the UI viewport. It is deliberately larger than the card-only progress-zero rectangle so scaling or clipping cannot make an apparently visible waiting card unclickable. A geometry regression requires the hitbox to contain both the reader-hotspot centre and visible-card centre.

Free-reader blank-card hotspot: the right-edge reader remains a secondary-click target for a new blank card whenever no card is physically in the transport, including while an earlier card is parked at the left exit or displayed in the passive holder. `ProgramCardView::reader_free_for_new_blank` carries that host fact into the renderer. Waiting/reading phases suppress the new-card hotspot because the reader is occupied.

PDF-derived face textures: `ProgramCardView::face_texture` optionally carries a raster card face generated from an original pack manual. The renderer maps that texture over the same chamfered physical-card polygon used by the procedural face. The raster rows themselves are fully opaque on a black canonical substrate; the mesh polygon, not PNG transparency, owns the physical chamfer. While the card is inside the passive holder, a black trapezoidal slot backing is painted only beneath the visible card and terminates just before the sloping photographed right rail. This prevents bright metal from appearing through the cut corners without hiding the rail's white highlight. End-2 reinsertion flips the texture UVs by 180 degrees, so exact manual artwork follows the same physical orientation rule as labels, marks and the HP logo. When no face texture is present, the existing procedural rendering path remains unchanged.
