...
## Implementation
The ignored test uses `std::time::Instant` around repeated continuous workloads after a warm-up phase. Additional rows expose architectural execution by itself, the same dual architectural+structural arrangement used by the live machine, and a versioned-ROM firmware stream, so structural headroom cannot hide host-side work or an unrealistically trivial ROM/control-flow fixture. It keeps one backplane/device state alive for every measured round, summarizes median/min/max durations, converts them to microseconds per 56-bit word and words per second, and compares the median with the observed `HP67_OBSERVED_WORD_TIME_US` physical reference. `std::hint::black_box` prevents trivial elimination of measured work without adding per-edge observer overhead.
