...
The tool mirrors the documented host-media layout implemented in `src/machines/hp67/magnetic_card.rs`: 34 28-bit records per logical track, 119 packed bytes per track, and the 250-byte `HP67CARD` v1 container. Its Teenix fixtures are consumed by `src/program_library.rs`; the sibling native `.hp67card` files are regression or interchange fixtures for the same logical media.
