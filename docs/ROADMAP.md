...
- **M14 — in progress:** M14A locks the source-backed timing/evidence contract; M14B places the continuous 56-bit DATA phase in the shared structural word; M14C moves installed-RAM ACT read/write destinations onto reconstructed DATA frames; M14D makes the ADD/SUB-family result image accumulated by the actual b0..b55 structural traversal authoritative for final A/B/C/carry at the completed word boundary, with the instruction-boundary executor retained only as an oracle for those values and as the temporary owner of still-unmigrated effects. Exact DATA polarity/passive bias, PHI-relative launch/sample/commit edges, per-device propagation, physical 1818-* RAM partitioning and true intra-word ACT register/carry mutation remain source-blocked or pending.
