...
- **M14 — in progress:** M14A locks the source-backed timing/evidence contract; M14B places the continuous 56-bit DATA phase in the shared structural word; M14C moves installed-RAM ACT read/write destinations onto reconstructed DATA frames while retaining the instruction-boundary executor only as an oracle for those transfer values. Exact DATA polarity/passive bias, PHI-relative launch/sample/commit edges, per-device propagation, physical 1818-* RAM partitioning and full ACT intra-word mutation remain source-blocked or pending.
