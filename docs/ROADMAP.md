...
- **M13 — complete (2026-09-20):** one physical `Hp67MagneticCard` owns independent logical Track 1/Track 2 media and is selected end-for-end; `.hpp`, `.hp67raw` and `.hp67card` host adapters remain outside the reader electronics; real firmware owns card-present→motor→head/startup handshake, two 28-bit CRC read buffers, two write buffers, `0x9B`/`0x99`, write-protect/F7 and the complete 34-record lifecycle; transport speed supports the documented 95–105% range without accumulated rounding drift; firmware write→eject→reinsert→read round trips are regression-locked for program headers 3/4 and W/DATA headers 1/2; the UI reader follows live record position and real firmware `Crd` drives same-card opposite-end reinsertion, with End2 rendered as a true 180° in-plane rotation; blank cards can be inserted host-side and written entirely through firmware; `Ctrl+S` persists returned media as lossless `.hp67card` or, when both tracks exist, `.hp67raw`. The physical self-clocking Zero/One flux-pair model is implemented separately. Three lower-level electrical facts remain deliberately source-blocked rather than guessed: HP-67/97 intra-record 28-bit serialization order, exact insertion-to-head geometry/acceleration, and sense-amplifier/PHI-relative bit timing. The built-in SD-14A artwork also remains unbound to magnetic media until a redistributable verified Moon Rocket Lander card image is inspected.
- **M14 — in progress:** M14A locks the source-backed timing/evidence contract; M14B places the continuous 56-bit DATA phase in the shared structural word; M14C moves installed-RAM ACT read/write destinations onto reconstructed DATA frames; M14D (validated locally 2026-09-24, diagnostic 12/12) makes ADD/SUB structural-result authoritative for final A/B/C/carry; M14E (validated locally 2026-09-25, diagnostic 12/12) extends that authority to all 32 arithmetic opcodes; M14F moves the focused P/status condition family (P, P-change history, status bits, condition carry, previous-carry and ThenGoto) onto a completed-word structural result image while retaining the architectural executor only as oracle and temporary owner of still-unmigrated control effects. Exact DATA polarity/passive bias, PHI-relative launch/sample/commit edges, per-device propagation, physical 1818-* RAM partitioning and true intra-word ACT register/control mutation remain source-blocked or pending.
- **M15 — planned:** full hardware validation suite.

M11 closure evidence is recorded in `docs/research/M11_FIDELITY_AUDIT_2026-09-18.md`. M12 program-entry/execution evidence is recorded in `docs/research/M12_PROGRAM_EXECUTION_AUDIT_2026-09-18.md`; PROGRAM editing and SST/BST closure evidence is recorded in `docs/research/M12_PROGRAM_EDIT_STEP_AUDIT_2026-09-18.md`; the code-cleanup review is recorded in `docs/research/M12_CODE_AUDIT_2026-09-18.md`. M13 presentation-slice evidence is recorded in `docs/research/M13_CARD_UI_PRESENTATION_2026-09-19.md`; timed transport/read-boundary evidence is recorded in `docs/research/M13_CARD_TRANSPORT_2026-09-19.md`. Final closure, including the real-corpus Teenix compatibility fixes and the successful local formatting/warnings/all-targets/release gate on 2026-09-20, is recorded in `docs/research/M13_CLOSURE_AUDIT_2026-09-19.md`. M14D's ADD/SUB final-state authority and remaining source-blocked ACT timing boundary are recorded in `docs/research/M14D_SERIAL_ARITHMETIC_AUTHORITY_2026-09-24.md`; M14E's extension to the complete arithmetic opcode family is recorded in `docs/research/M14E_SERIAL_REGISTER_AUTHORITY_2026-09-25.md`; M14F's P/status authority boundary is recorded in `docs/research/M14F_P_STATUS_AUTHORITY_2026-09-25.md`.
...
- **M3 — partial and currently the main fidelity bottleneck.** Instruction-boundary ACT semantics execute the real firmware to idle; ACT owns display serialization, ROM address serialization, ROM-word sampling and the fifteen-word display phase. The old A/B nibble snapshot is gone, and the currently executing prefetched instruction now has an explicit lifetime across the same complete b0..b55 structural word that fetches its successor. M14E makes the structural result image authoritative for final A/B/C/carry across all 32 arithmetic opcodes, and M14F begins migrating non-arithmetic ACT state with completed-word authority for the focused P/status condition family. The transport refuses to replace an incomplete execution. The major missing piece remains source-backed intra-word mutation/observability on real bit/PHI timing plus migration of the remaining ACT control/state families.
