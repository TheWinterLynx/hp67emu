...
- **M13 — in progress (2026-09-19):** top-down card presentation uses one 71.1 × 11.4 mm physical card geometry; CRC external `card_present` and firmware-owned `motor_on` are regression-locked; nominal magnetic transport now advances at the documented 28 ms per 28-bit record cadence behind a separate head-active gate; CRC `buffer_ready` is transport-driven; architectural read port `0x9B` consumes the 28-bit buffer; and the branch now implements a two-buffer `0x99` write FIFO, nominal 28 ms write draining and per-side write protection with firmware F7 status. The timed read and write slices have both passed the full local gate; complete 34-record media write→eject→reinsert→read round-trip regressions have passed the full local gate. Live tests keep motor-switch/card-present separate from the later head contact, and the transport now performs the shared CRC/head startup-ready handshake before beginning the 28 ms magnetic-record cadence. `Hp67LiveMachine` now owns the firmware-synchronized card-present→motor→head→startup-ready→34-record→card-exit lifecycle without test-side head toggling. A verified magnetic payload still needs to be associated with the visual SD-14A card before the UI can drive the real reader; exact physical head geometry and lower-level sense/DATA timing remain to implement.
